...
export interface CombatSettings {
  ...
  selfDefenseMode: boolean;
  targetPlayers: boolean;
  targetMobs: boolean;
  maxAttackDistance: number;
  fleeWhenLowHealth: boolean;
  healthThreshold: number;
  retaliationEnabled: boolean;
}
...
export class CombatSystem {
  ...
  private recentAttackers: Set<string> = new Set();
  private autoStopAfterKill = true; // New setting to auto-stop after killing target
  ...
  private onTakeDamage(): void {
    try {
      if (!this.settings.retaliationEnabled && !this.settings.selfDefenseMode) return;
  ...
  private findBestTarget(): Entity | null {
    ...
    const potentialTargets: AttackTarget[] = [];

    for (const entity of entities) {
      if (!this.shouldTargetEntity(entity)) continue;

      const distance = this.bot.entity.position.distanceTo(entity.position);
      if (distance > this.settings.maxAttackDistance + 5) continue;
  ...
  private shouldTargetEntity(entity: Entity): boolean {
    try {
      if (!entity || entity === this.bot.entity || !entity.position) return false;

      // Check if entity is a player
      if (entity.type === 'player') {
        if (!this.settings.targetPlayers) return false;

        // Always target recent attackers
        if (entity.username && this.recentAttackers.has(entity.username)) return true;
  ...
  private calculateThreatLevel(entity: Entity): number {
    let threat = 0;
    ...
    if (entity.type === 'player') {
      ...
      // Mob threat levels
      const mobThreats: { [key: string]: number } = {
        'creeper': 150,
        'skeleton': 80,
        'zombie': 60,
        'spider': 40,
  ...
  private attackTarget(target: Entity): void {
    try {
      if (!target || !target.position || !this.bot.entity) return;
  ...
  private moveTowardsTarget(target: Entity): void {
    const goal = new goals.GoalNear(
      target.position.x,
      target.position.y,
      target.position.z,
      this.settings.maxAttackDistance - 1
    );
  ...
  private setSpecificTarget(playerName: string): void {
  ...
  private stopCombatAfterKill(): void {
  ...
  private stopCombatAfterDeath(): void {
  ...
  updateSettings(newSettings: Partial<CombatSettings>): string {
