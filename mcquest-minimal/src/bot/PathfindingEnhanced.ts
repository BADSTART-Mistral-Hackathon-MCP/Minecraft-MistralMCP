...
const { goals, Movements } = pathfinderModule as unknown as typeof import('mineflayer-pathfinder');
...
export interface SafetyMetrics {
  dangerScore: number;
  safePath: boolean;
  dangerousBlocks: Array<{ name: string; position: { x: number; y: number; z: number } }>;
...
export class PathfindingEnhanced {
  ...
  private async attemptPathfinding(
    goal: any,
    ...
    return new Promise((resolve, reject) => {
      ...
      const onGoalReached = () => {
        cleanup();
        const path = (this.bot.pathfinder as any).path?.path || [];
  ...
  private createEnhancedMovements(options: PathfindingOptions): any {
    const movements = new Movements(this.bot);

    // Configure movement settings based on options
    (movements as any).allowFreeMotion = options.optimizeForSpeed;
    (movements as any).allow1by1towers = options.allowPlacing;
    (movements as any).allowEntityDetection = true;

    // Set block breaking/placing preferences
    if (options.allowBreaking) {
      movements.blocksToAvoid.clear();
      (movements as any).canDig = (block: Block) => {
        return !this.isValuableBlock(block.name) && this.bot.canDigBlock(block);
      };
    }

    if (options.allowPlacing) {
      (movements as any).canPlace = (block: Block) => {
  ...
  private async analyzeSafety(
    ...
    const dangerousBlocks: Array<{ name: string; position: { x: number; y: number; z: number } }> = [];
  ...
  private calculatePathDistance(path: Array<{ x: number; y: number; z: number }>): number {
  ...
  private getDangerScore(blockName: string): number {
    if (blockName.includes('lava')) return 10;
    if (blockName.includes('fire')) return 8;
    if (blockName.includes('magma')) return 6;
    if (blockName.includes('cactus')) return 4;
    return 2;
  }
  ...
  async findPathToNearestBlock(blockType: string, maxDistance: number = 64): Promise<PathfindingResult> {
  ...
  async findPathToPlayer(playerName: string): Promise<PathfindingResult> {
  ...
  async findPathToSafeLocation(): Promise<PathfindingResult> {
  ...
  private calculateLocationSafety(pos: { x: number; y: number; z: number }): number {
    let score = 100;
    const radius = 5;
      ...
      for (let dy = -radius; dy <= radius; dy++) {
        for (let dz = -radius; dz <= radius; dz++) {
          const block = this.bot.blockAt(this.bot.entity.position.offset(
            pos.x - this.bot.entity.position.x + dx,
            pos.y - this.bot.entity.position.y + dy,
            pos.z - this.bot.entity.position.z + dz
          ));
