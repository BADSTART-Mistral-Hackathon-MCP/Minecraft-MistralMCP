...
export interface SentinelSettings {
  ...
  zoneCenter: { x: number; y: number; z: number } | null;
  zoneRadius: number;
...
export class SentinelSystem {
  ...
  private protectedPlayerEntity: Entity | null = null;
  private threats: Set<Entity> = new Set();
  private lastEatTime = 0;
  private lastEquipCheck = 0;
  ...
  private setupEventListeners(): void {
    try {
      ...
      this.bot.on('entityHurt', (entity) => {
        try {
          if (!this.isActive || !entity) return;
  ...
  setZoneDefense(center: { x: number; y: number; z: number }, radius: number): string {
    try {
  ...
  private findProtectedPlayer(): void {
    try {
      if (!this.settings.protectedPlayer) return;
  ...
  private defendProtectedPlayer(): void {
    try {
      if (!this.protectedPlayerEntity) return;
  ...
  private findThreatsNear(position: any, radius: number): Entity[] {
    try {
  ...
  private engageThreat(threat: Entity): void {
    try {
  ...
  private checkProtectedPlayer(): void {
    try {
      if (!this.protectedPlayerEntity || !this.settings.protectedPlayer) return;

      // Check if player is still online
      const currentPlayer = Object.values(this.bot.entities).find(entity =>
        entity.type === 'player' &&
        entity.username === this.settings.protectedPlayer
      );
  ...
  private checkZoneIntruders(): void {
    try {
      if (!this.settings.zoneCenter) return;
      ...
      Object.values(this.bot.entities).forEach(entity => {
        ...
        const distance = this.settings.zoneCenter!.x !== undefined ?
          Math.sqrt(
            Math.pow(entity.position.x - this.settings.zoneCenter!.x, 2) +
            Math.pow(entity.position.z - this.settings.zoneCenter!.z, 2)
          ) : Infinity;
  ...
  private checkAndEquipGear(): void {
    try {
  ...
  private equipBestArmor(): void {
    try {
      const armorSlots = [
        { name: 'head', destination: 'head' as const },
        { name: 'torso', destination: 'torso' as const },
        { name: 'legs', destination: 'legs' as const },
        { name: 'feet', destination: 'feet' as const }
      ];
  ...
  private checkAndEat(): void {
    try {
      if (!this.settings.autoEat) return;
      if (this.bot.food >= this.settings.hungerThreshold) return;
  ...
  private getArmorSlotId(slot: string): number {
  ...
  private findBestArmorForSlot(slot: string): any {
    try {
      const armorTypes: { [key: string]: string[] } = {
        'head': ['helmet', '_helmet'],
  ...
  private getArmorValue(item: any): number {
  ...
  private getWeaponDamage(item: any): number {
    ...
    const weaponDamage: { [key: string]: number } = {
      'wooden_sword': 4,
      'stone_sword': 5,
      'iron_sword': 6,
      'diamond_sword': 7,
  ...
  private getFoodValue(item: any): number {
    if (!item || !item.name) return 0;
  ...
  private evaluateNewEntity(entity: Entity): void {
    try {
  ...
  updateSettings(newSettings: Partial<SentinelSettings>): string {
    try {
