...
import { z } from "zod";
import { init, setSeed, getBiome, listStructures, blockToChunk } from "./cubiomes";
...
app.get("/api/biome", async (req, res) => {
  ...
  try {
    await setSeed(result.data.seed, result.data.dim as 0 | 1 | 2);
    const biomeId = await getBiome({
      scale: result.data.scale as 1 | 4 | 16 | 64 | 256,
      x: result.data.x,
      z: result.data.z,
      y: result.data.y,
    });
